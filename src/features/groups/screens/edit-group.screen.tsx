...
import { updateGroupSchema, UpdateGroupSchemaType } from '../groups.dto';
import useUpdateGroup from '../hooks/use-update-group';
import useGroupDetail from '../hooks/use-group-detail';
import EditGroupHeader from '@/features/groups/components/edit-group.header';
...
export default function EditGroupScreen() {
  ...
  // Pre-populate fields once group data loads
  useEffect(() => {
    if (!group) return;
    form.reset({
      name: group.name ?? '',
    ...
    if (group.emoji) setSelectedIcon(group.emoji);
    if (group.color) setSelectedColor(group.color);
  }, [group?.id]);
  ...
  return (
    <ScreenContainer>
      ...
      <View style={styles.footer}>
        ...
        >
          <Ionicons2 name="checkmark-done-outline" size={22} color={colors.text.inverse} />
          <Text style={[TextStyles.button, { color: colors.onPrimary }]}>Save</Text>
        </TouchableOpacity>
      </View>
    </ScreenContainer>
  );
